...
import {
  listSingboxInbounds, fetchSingboxTrafficBatch,
  type SingboxInbound,
} from '@/api/plugins'
...
export default function TrafficTab() {
  ...
  const trafficQ = useQuery({
    queryKey: ['singbox-traffic-tab', range.key, allTags],
    queryFn: async () => {
      const now = new Date()
      ...
      const results = await Promise.all(
        Array.from(tagsByServer.entries()).map(([serverID, tags]) =>
          fetchSingboxTrafficBatch({ server_id: serverID, tags, from, to, resolution: range.resolution })
        )
      )
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {t('singbox.traffic.description', 'Inbound traffic — uplink + downlink by inbound tag. Click any row for detailed chart.')}
        </p>
            ...
            <Table wrapperClassName="border-0 rounded-none bg-transparent">
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead>{t('singbox.traffic.tag', 'Tag')}</TableHead>
                  <TableHead>{t('singbox.traffic.role', 'Role')}</TableHead>
                  <TableHead>{t('singbox.traffic.last_range', 'Last {{range}}', { range: range.label })}</TableHead>
                  <TableHead className="text-right">{t('singbox.traffic.uplink', 'Uplink')}</TableHead>
                  <TableHead className="text-right">{t('singbox.traffic.downlink', 'Downlink')}</TableHead>
                  <TableHead className="text-right">{t('singbox.traffic.total', 'Total')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {inbounds.map((i) => {
      ...
      {(inboundsQ.data ?? []).length === 0 && (
        <div className="rounded-lg border bg-elev p-6 text-center text-muted-foreground text-sm">
          {t('singbox.empty.traffic', 'No inbounds deployed yet. Create some in the Inbounds tab.')}
        </div>
      )}
