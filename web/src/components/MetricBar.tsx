...
export function MetricBar({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-fg-dim text-2xs w-[30px] shrink-0">{label}</span>
      ...
      <span
        className={cn('font-mono tabular-nums text-2xs w-[34px] text-right', tone === 'warn' && 'text-warn', tone === 'err' && 'text-err')}
      >
        {value.toFixed(0)}%
      </span>
    </div>
