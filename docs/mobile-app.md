...
### 4.1 服务器墙（首页）
参考 web 端「公开墙」的形态：
- 顶部**汇总条**：节点总数 / 在线 / 离线 / **实时 ↓↑**（实时聚合带宽）。
- 按**分组**（`public_group`）聚合，每组标题显示 `在线/总数`，无分组归入 Ungrouped。
- 每台机器一张**卡片**：在线点 · 国旗 · 别名 · 平台·架构 · CPU/MEM/DSK 进度条 ·
  **实时 ↓↑** · load。在线的排在前面，离线卡片变灰。
- **实时流量**：通过一条公开多路 WebSocket（`/api/public/net-live/ws`）每秒推送，
  卡片与汇总条的 ↓↑ 实时跳动；未开启公开展示的机器回退到 5 秒轮询值。断线会自动重连
  （退避 + 回到前台立即重连）。
- 列表每 5 秒轮询刷新指标；下拉可手动刷新。点卡片进入**服务器详情**（详情页的网络
  读数同样实时）。
- 全屏适配：页面顶栏与底部都已避开刘海 / 状态栏 / Home 指示条。
