      ...
      <div className="px-3.5 py-2">
        ...
        {items.map((item, i) => (
          <button
            key={item.id}
            className={
              'w-full flex items-center gap-3 py-2 text-left cursor-pointer hover:bg-sunken/60 transition-colors ' +
              (i < items.length - 1 ? 'border-b border-dashed' : '')
            }
            onClick={() => navigate(`${linkBase}/${item.id}`)}
          >
            <span className="font-mono text-sm flex-1 min-w-0 truncate text-foreground">
              {item.name}
            </span>
            <Sparkline
            ...
            />
            <span className="font-mono tabular-nums text-sm font-medium min-w-[48px] text-right shrink-0">
              {item.value}
            </span>
          </button>
        ))}
