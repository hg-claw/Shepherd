...
export default function ScriptRunDetailPage() {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <PageHeader title={<>{t('scripts.run', 'Run')} #{id}</>} />
      ...
      {targets.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          {okCount > 0 && (
            <span className="inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full bg-ok-soft text-ok text-2xs font-mono border border-transparent">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-ok" />
              {okCount} succeeded
            </span>
          )}
          {runCount > 0 && (
            <span className="inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full bg-warn-soft text-warn text-2xs font-mono border border-transparent">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-warn shep-pulse-warn" />
              {runCount} running
            </span>
          )}
          {failCount > 0 && (
            <span className="inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full bg-err-soft text-err text-2xs font-mono border border-transparent">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-err" />
              {failCount} failed
            </span>
          )}
          {pendCount > 0 && (
            <span className="inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full bg-sunken text-muted-foreground text-2xs font-mono border border-border">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-muted-foreground" />
              {pendCount} pending
            </span>
          )}
          <span className="text-fg-dim font-mono text-2xs ml-1">
      ...
      <div className="border rounded-lg bg-elev overflow-hidden">
        <div className="flex items-center gap-2 px-3.5 py-2.5 border-b">
          ...
          <span className="text-fg-dim font-mono text-2xs ml-auto">{targets.length} servers</span>
        </div>
