...
const patchFn = vi.fn().mockResolvedValue({ ok: true, version: '1.8.20' })
...
vi.mock('@/api/plugins', () => ({
  listPluginHosts: vi.fn().mockResolvedValue([
    ...
    },
    {
      ...
      server_id: 2,
      config: {},
      deployed_version: '1.8.10',
      status: 'failed',
      last_error: 'connection refused: dial tcp 1.2.3.4:22',
...
describe('xray/DeployTab', () => {
  ...
  it('shows "see error" tooltip for failed host with last_error', async () => {
    ...
    await waitFor(() => {
      ...
      expect(errEl.getAttribute('title')).toContain('connection refused')
    })
  })
  ...
  it('clicking Re-deploy calls patchXrayServerVersion with current version', async () => {
    const user = userEvent.setup()
    render(<DeployTab />, { wrapper })
    const btns = await screen.findAllByText('Re-deploy')
    await user.click(btns[0])
    await waitFor(() => {
      expect(patchFn).toHaveBeenCalledWith(1, '1.8.11')
    })
  })
})
