...
export default function PluginDetail() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      <Pressable onPress={() => router.push(('/(app)/plugins/' + p.id + '/config') as never)} style={rowStyle}>
        <Text style={{ color: theme.accent }}>Edit config</Text>
      </Pressable>
      {p.meta.host_aware ? (
        <Pressable onPress={() => router.push(('/(app)/plugins/' + p.id + '/hosts') as never)} style={rowStyle}>
